...
- **Rule**: You can only return _one_ parent element (like a `<div>`).
- **Dynamic Data**: You can put JavaScript variables inside `{}`. Example: `<h1>{title}</h1>`.

### 3. Props (Properties)

Props are how we pass data _down_ from a parent component to a child component.
...
Effects let you run code when something happens _outside_ of just showing HTML (side effects).
...
- **State (`view`)**: `const [view, setView] = useState<'list' | 'form'>('list');`
  - We use this to decide whether to show the _List_ or the _Form_.
- **`useEffect`**:
  - When the app loads, it immediately calls `loadExpenses()` to fetch data from the Laravel API.
- **`handleFormSubmit`**:
  - This function is passed _down_ to the `ExpenseForm`. When the form is submitted, this function runs to actually save the data to the server (via `api.ts`) and refresh the list.
